...
| 4 | **CPV-code filtering** | Hilma side: CPV-filtered. Tarjouspalvelu side: NOT CPV-filtered — only filtered by notice type. CPV tags arrive only when a tarjouspalvelu tender also exists in Hilma (via `merge.py`) | 🟡 Decide: (a) add post-scrape CPV inference from description+keywords, (b) accept tarjouspalvelu-only tenders without CPV, or (c) build a CPV lookup step |
...
| 7 | Preliminary analysis — **question deadline + tender deadline** | Only tender deadline (`deadline`) captured as structured field. **Question deadline NOT separately persisted** (it appears in raw detail text but isn't extracted) | 🔴 Add: extract "Kysymysten jättö määräaika" during tarjouspalvelu tab scrape; add `question_deadline` field |
| 8 | Preliminary analysis — **estimated size** (kokoluokka) | Captured from Hilma as `estimated_value`; tarjouspalvelu-only tenders don't get this | 🟡 Acceptable for MVP (most big tenders are in Hilma); consider AI extraction fallback later |
| 9 | Preliminary analysis — **restricted vs. open procedure** | Not extracted | 🔴 Add: AI extraction from description + attachments, OR structural field from Hilma's `procedureType` |
| 10 | Preliminary analysis — **scoring mechanism (quality vs. price)** | Not extracted | 🔴 Add: AI extraction from the "Muut ehdot" tab on tarjouspalvelu |
| 11 | Preliminary analysis — **contract included + reservations allowed** | Not extracted. Note: existing `contract_duration` / `contract_value` fields are for competitor analysis (winners), not this requirement. **Name collision risk** | 🔴 Add: AI extraction; use distinct field names like `contract_included` (bool) + `reservations_allowed` (bool) to avoid confusion with competitor fields |
...
**Summary:**
- ✅ 6 items work as-is
- 🟡 7 items need decisions or minor config changes
- 🔴 5 items require new code (SharePoint upload, question deadline extraction, procedure type extraction, scoring-mechanism extraction, contract/reservations extraction, plus Defender enablement)
